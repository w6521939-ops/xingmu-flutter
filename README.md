...
- 成片元数据与演示预览；真实视频播放、下载和系统分享仍是后续能力
- 不联网、不收费的确定性演示模式
...
本仓库目前只提供移动客户端与契约，不包含可公开部署的百炼代理后端或 Wan2.2 权重。
...
状态只使用 `passed`、`failed`、`not run`。以下记录对应 2026-08-20 的当前工作区，不代表 CI 或真实设备结果：

| 检查 | 状态 | 说明 |
| --- | --- | --- |
| CPF-Flutter 版本 | passed | `3.35.8-ohos-1.0.3-beta`，CPF GitCode fork，Dart 3.9.2 |
| OpenAPI YAML 解析 | passed | OpenAPI 3.1.0，23 paths、40 schemas、208 个本地引用，路径参数一致；尚未运行语义 linter |
| `flutter analyze` | passed | 0 issues |
| `flutter test` | passed | 113 tests |
| Android debug APK | passed | 149.07 MiB，`build/app/outputs/flutter-apk/app-debug.apk` |
| OpenHarmony Hvigor unsigned HAP | passed | API 20 编译打包成功；131.98 MiB，产出 `entry-default-unsigned.hap`；CPF 依赖仍有 ArkTS/deprecation warnings |
| OpenHarmony signed debug HAP | failed | 本机尚未配置 DevEco 调试签名；仓库不会提交签名材料 |
| Android 真机/模拟器 | not run | 尚未设备验证 |
| OpenHarmony API 20 真机/模拟器 | not run | 尚未设备验证 |
